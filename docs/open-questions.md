...
- **Worker concurrency:** How many queue items processed in parallel? Likely 1–3. Tune during Phase 1. (Phase 2 runs serial — 1 at a time.)
- **Single bulk-enumeration point (decided in principle 2026-06-14, implement at Phase 2 chunk 6):** Full-library reads from Calibre REST (`calibre.fetch_library`, ~30 batched `/ajax/books` requests over 7,344 books) should happen in exactly **one** place — building/refreshing the snapshot SQLite. Every other "what's already in the library" lookup (ship normalization, X4 catalog, etc.) reads the local snapshot DB instead — e.g. existing ship values become `SELECT DISTINCT primaryship FROM books`, instant and network-free. The snapshot is the canonical full-library read-model (it exists so clients don't hammer Calibre); the worker should use it the same way. Safe because non-snapshot consumers only need *existing* (prior-book) data, which the snapshot already holds, and Phase 2 is serial. Effect: `calibre.fetch_existing_ship_values()` becomes a **bootstrap-only** helper (needed before the first snapshot exists), not the per-import hot path.
